  ...
  return (
    <div className="container">
      <div className="container-selector">
        <h1>Enter the URL of a website to track:</h1>
        <div className="btn-container">
          <Input value={url} onChange={e => setUrl(e.target.value)} placeholder="Website URL" />
        </div>
      </div>
      <div className="container-selector">
        <h1>Enter an interval to check for changes:</h1>
        <Select
          style={{ width: '100%' }}
      ...
      <Button className="btn-track" onClick={handleStartTracking} style={{ color: '#fff' }}>
        Track
      </Button>
      <List
        ...
        renderItem={website => (
          <List.Item
            ...
            style={{ border: '1px solid white', borderColor: hasChanged[website] ? 'red' : 'green' }}>
            <List.Item.Meta
              avatar={
                <img
                  style={{
              ...
              title={website}
              description={`Counter: ${checkCount[website] || 0}, Next check in: ${
                intervalTime - ((checkCount[website] || 0) % intervalTime)
              }s`}
            />
          </List.Item>
        )}
